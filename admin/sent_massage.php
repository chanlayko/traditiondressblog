<?php 
    ...
    if(empty($_POST['search']) && empty($_COOKIE['search'])){
      ...
      $stat = $pdo -> prepare("SELECT * FROM send_mail WHERE author_id=$author_id AND sayadmin='' AND draft='sent' ORDER BY id DESC");
      $stat -> execute();
      ...
      $stat = $pdo -> prepare("SELECT * FROM send_mail WHERE author_id=$author_id AND sayadmin='' AND draft='sent' ORDER BY id DESC LIMIT $offset,$numOfrecs");
      $stat -> execute();
      $result = $stat -> fetchAll();
      ...
      $stat = $pdo -> prepare("SELECT * FROM send_mail WHERE author_id=$author_id AND sayadmin='' AND draft='sent' AND mail LIKE '%$searchKey%' ORDER BY id DESC");
      $stat -> execute();
      $result = $stat -> fetchAll();
      ...
      $stat = $pdo -> prepare("SELECT * FROM send_mail WHERE author_id=$author_id AND sayadmin='' AND draft='sent' AND mail LIKE '%$searchKey%' ORDER BY id DESC LIMIT $offset,$numOfrecs");
      $stat -> execute();
      $result = $stat -> fetchAll();
      ...
      if (isset($_POST['checkboxArray'])) {
        foreach ($_POST['checkboxArray'] as $checkArray) {
          ...
          if ($Resul[0]['gift_order'] == 'gift' || $Resul[0]['gift_order'] == 'pickup') {
            ...
            if ($teResult) {
              echo "<script>alert('Sussessfully Deleted Updated');window.location.href='sent_massage.php';</script>";
            }
          }else{

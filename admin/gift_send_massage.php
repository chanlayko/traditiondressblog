<?php 
    ...
    if (isset($_POST['send'])) {
      ...
      $Result = $stat -> execute([':mail'=>$mail,':subject'=>$subject,':massage'=>$massage,':draft'=>'sent',':order_method'=>'gift',':mail_star'=>$star,':order_id'=>$gift_id,':author_id'=>$author_id]);
    ...
    if (isset($_POST['draft'])) {
      ...
      $stat = $pdo -> prepare("INSERT INTO send_mail(mail,subject,massage,draft,order_method,mail_star,order_id,author_id) VALUES (:mail,:subject,:massage,:draft,:order_method,:mail_star,:order_id,:author_id)");
      $Result = $stat -> execute([':mail'=>$mail,':subject'=>$subject,':massage'=>$massage,':draft'=>'draft',':order_method'=>'gift',':mail_star'=>$star,':order_id'=>$gift_id,':author_id'=>$author_id]);
  ...
  <!-- /.navbar -->
    ...
    <section class="content">
      <div class="container-fluid">
        <div class="row">
          <div class="col-md-3">
            <a href="gift_information.php" class="btn btn-primary btn-block mb-3">Back to Gift Wrap Order</a>
            <div class="card">
                ...
                <ul class="nav nav-pills flex-column">
                  ...
                  <li class="nav-item">
                    <?php
                      ...
                      $spdo = $pdo -> prepare("SELECT * FROM sal_order WHERE sal_user_id=$gift_id");
                      $spdo -> execute();
                      $result = $spdo -> fetchAll();
                      $result_count = count($result);
                    ?>
                    <a class="nav-link" href="gift_massage.php?gi_id=<?php echo $pdoResult['id']; ?>"><i class="far fa-circle text-success"></i> <?php echo escape($result_count); ?> Post Count</a>
                  </li>
                </ul>
              </div>
              <!-- /.card-body -->

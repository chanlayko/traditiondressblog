<?php 
    ...
    if(empty($_POST['search']) && empty($_COOKIE['search'])){
      ...
      $stat = $pdo -> prepare("SELECT * FROM send_mail WHERE author_id=$author_id AND sayadmin='sayadmin' AND draft='draft' ORDER BY id DESC");
      $stat -> execute();
      ...
      $stat = $pdo -> prepare("SELECT * FROM send_mail WHERE author_id=$author_id AND sayadmin='sayadmin' AND draft='draft' ORDER BY id DESC LIMIT $offset,$numOfrecs");
      $stat -> execute();
      $result = $stat -> fetchAll();
      ...
      $stat = $pdo -> prepare("SELECT * FROM send_mail WHERE author_id=$author_id AND sayadmin='sayadmin' AND draft='draft' AND mail LIKE '%$searchKey%' ORDER BY id DESC");
      $stat -> execute();
      $result = $stat -> fetchAll();
      ...
      $stat = $pdo -> prepare("SELECT * FROM send_mail WHERE author_id=$author_id AND sayadmin='sayadmin' AND draft='draft' AND mail LIKE '%$searchKey%' ORDER BY id DESC LIMIT $offset,$numOfrecs");
      $stat -> execute();
      $result = $stat -> fetchAll();
      ...
      if (isset($_POST['checkboxArray'])) {
        foreach ($_POST['checkboxArray'] as $checkArray) {
          ...
          // if ($Resul[0]['gift_order'] == 'gift' || $Resul[0]['gift_order'] == 'pickup') {
            ...
            if ($teResult) {
              echo "<script>alert('Sussessfully Deleted Updated');window.location.href='draft_sayadmin.php';</script>";
            }
          // }else{
            ...
            // $teResult = $tesql -> execute();
            // if ($teResult) {
            //   echo "<script>alert('Sussessfully Deleted Updated');window.location.href='draft_massage.php';</script>";
            // }
          // }
        }
      }
        ...
        <div class="col-md-9">
          <div class="card card-primary card-outline">
            <div class="card-header">
              ...
              <form action="draft_massage.php" method="post">
                    ...
                    <tr>
                      <td>
                        <div class="icheck-primary">
                          <input type="checkbox" class="checkbox" name="checkboxArray[]" value="<?php echo $value['id']; ?>" id="check1">

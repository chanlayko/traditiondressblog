<?php 
    ...
    if (isset($_POST['send'])) {
      ...
      $Result = $stat -> execute([':mail'=>$mail,':subject'=>$subject,':massage'=>$massage,':draft'=>'sent',':order_method'=>'delivery',':mail_star'=>$star,':order_id'=>$order_id,':author_id'=>$author_id]);
    ...
    if (isset($_POST['draft'])) {
      ...
      $stat = $pdo -> prepare("INSERT INTO send_mail(mail,subject,massage,draft,order_method,mail_star,order_id,author_id) VALUES (:mail,:subject,:massage,:draft,:order_method,:mail_star,:order_id,:author_id)");
      $Result = $stat -> execute([':mail'=>$mail,':subject'=>$subject,':massage'=>$massage,':draft'=>'draft',':order_method'=>'delivery',':mail_star'=>$star,':order_id'=>$order_id,':author_id'=>$author_id]);
            ...
            <div class="card">
              ...
              <div class="card-body p-0">
                <ul class="nav nav-pills flex-column">
                  <li class="nav-item">
                    <a class="nav-link" href="#"><i class="far fa-circle text-danger"></i> <?php echo escape($pdoResult['first_name'] .' '.$pdoResult['last_name']); ?><i class='fas fa-star text-warning float-right' id="astar"></i></a>
                  </li>
                  <li class="nav-item">
                  ...
                  <li class="nav-item">
                    <a class="nav-link" href="view_massage.php?od_id=<?php echo $pdoResult['id']; ?>"><i class="far fa-circle text-success"></i> <?php echo $sal_count; ?> Post Count</a>
                  </li>
                </ul>
              </div>
              <!-- /.card-body -->

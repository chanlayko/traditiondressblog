<?php 
    ...
    if(empty($_POST['search']) && empty($_COOKIE['search'])){
      ...
      $stat = $pdo -> prepare("SELECT * FROM send_mail WHERE author_id=$author_id AND sayadmin='sayadmin' AND order_method='income' ORDER BY id DESC");
      $stat -> execute();
      ...
      $stat = $pdo -> prepare("SELECT * FROM send_mail WHERE author_id=$author_id AND sayadmin='sayadmin' AND order_method='income' ORDER BY id DESC LIMIT $offset,$numOfrecs");
      $stat -> execute();
      $result = $stat -> fetchAll();
      ...
      $stat = $pdo -> prepare("SELECT * FROM send_mail WHERE author_id=$author_id AND sayadmin='sayadmin' AND order_method='income' AND mail LIKE '%$searchKey%' ORDER BY id DESC");
      $stat -> execute();
      $result = $stat -> fetchAll();
      ...
      $stat = $pdo -> prepare("SELECT * FROM send_mail WHERE author_id=$author_id AND sayadmin='sayadmin' AND order_method='income' AND mail LIKE '%$searchKey%' ORDER BY id DESC LIMIT $offset,$numOfrecs");
      $stat -> execute();
      $result = $stat -> fetchAll();
    ...
    if (isset($_POST['deleteSent'])) {
      ...
      if (isset($_POST['checkboxArray'])) {
        foreach ($_POST['checkboxArray'] as $checkArray) {
          ...
          $ats = $pdo -> prepare("SELECT * FROM admin_user");
          $ats -> execute();
          $Resul = $ats -> fetchAll();

          // if ($Resul[0]['gift_order'] == 'gift' || $Resul[0]['gift_order'] == 'pickup') {
            
            $name = $Resul[0]['username'];
            $mail = $Results[0]['mail'];
            ...
            $Result = $stat -> execute([':delete_id'=>$checkArray,':name'=>$name,':mail'=>$mail,':subject'=>$subject,':sayadmin'=>'admin',':author_id'=>$author]);

            $tesql = $pdo -> prepare("DELETE FROM send_mail WHERE id=$checkArray");
            $teResult = $tesql -> execute();
            if ($teResult) {
              echo "<script>alert('Sussessfully Deleted Updated');window.location.href='send_sayadmin.php';</script>";
            }
          // }else{
        ...
        <div class="col-md-9">
          <div class="card card-primary card-outline">
            <div class="card-header">
              ...
              <form action="sent_massage.php" method="post">
                    ...
                    <tr>
                      <td>
                        <div class="icheck-primary">
                          <input type="checkbox" name="checkboxArray[]" class="checkbox" value="<?php echo $value['id']; ?>" id="check1">
                          <label for="check1"></label>
                        </div>
                      </td>
                      <td class="mailbox-star"><a title="Important"><?php echo $value['mail_star'] == 1 ? "<i class='fas fa-star text-warning'></i>" : " " ; ?></a></td>
                      <td class="mailbox-name"><a href="read_massage.php?massage_id=<?php echo $value['order_id']; ?>&id=<?php echo $value['id']; ?>"><?php echo $value['order_method'] == 'income'? '<i class="fas fa-reply" style="color:red;"></i> Incoming' : '<i class="fas fa-share" style="color:green;"></i> Sending' ?></a></td>
                      <td class="mailbox-subject"><b><?php echo escape($value['subject']); ?></b> - <?php echo escape(substr($value['massage'], 0,80)) ?> . . . 
                      </td>
                      <td class="mailbox-attachment"><?php echo escape($value['draft']) == 'draft'? '<i class="fas fa-filter"></i>' : ''; ?></td>
